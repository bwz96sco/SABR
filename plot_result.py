...
from config import VIDEO_BIT_RATE, REBUF_PENALTY, LOG_FILE_DIR


# RESULTS_FOLDER = './test_results/baselines/'#LOG_FILE_DIR#'./test_results/'
RESULTS_FOLDER = LOG_FILE_DIR#'./test_results/'
# RESULTS_FOLDER = './test_results/4g_retrain/'#'./test_results/'
...
VIDEO_BIT_RATE = VIDEO_BIT_RATE#[1000,2500,5000,8000,16000,40000]  # Kbps
K_IN_M = 1000.0
# REBUF_P = 4.3
REBUF_P = REBUF_PENALTY#40
SMOOTH_P = 1
...
# SCHEMES = ['sim_bb', 'sim_rl', 'sim_mpc', 'sim_cmc', 'sim_bola', 'sim_netllm', 'sim_quetra', 'sim_genet', 'sim_ppo']
# SCHEMES = ['sim_bb', 'sim_rl', 'sim_mpc', 'sim_cmc','sim_bola', 'sim_netllm', 'sim_quetra',  'sim_ppo']


# SCHEMES = ['sim_bs', 'sim_mfd']
...
def main():
    ...
    for log_file in log_files:
        if not any(scheme in log_file for scheme in SCHEMES):
            continue
        time_ms = []
        bit_rate = []
        buff = []
        ...
        # print log_file
        if os.path.isdir(RESULTS_FOLDER + log_file): 
            continue
                ...
                for line in reversed(lines):
                    ...
                    if len(parse) > 1:
                        ...
                        if b == 4:
                            ...
                            assert rebuff >= -3e-4, f'rebuf {rebuff}'
                            r -= REBUF_P * rebuff
